...
The chat backend SHALL expose a server-local `web-fetch` tool to AI SDK `streamText(...)` so models that support tool calling can retrieve documentation content during a conversation, and the client SHALL surface those tool calls in the conversation UI.
